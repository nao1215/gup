...
每个元素都有以下字段：`name`、`import_path`、`module_path`、`channel`（`latest`/`main`/`master`）、`current_version`、`latest_version`（`list` 时为空）、`current_go_version`、`installed_go_version`、`status`，以及 `error`（不存在时省略）。`status` 可以是 `installed`（list）、`up-to-date`、`update-available`（check）、`updated`（update）或 `error`。
