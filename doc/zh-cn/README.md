...
gup 更新并管理您 `$GOBIN` 中全局的 Go 命令行工具。`go install` 会把每个程序放入 `$GOBIN`（`$GOPATH/bin`），但之后再也不会更新它，不会保留所安装内容的清单，也没有办法把某个工具固定在您所依赖的版本上。gup 则管理这整套工具：它并行地将整套工具更新到最新，可以把选定的工具 `pin`（固定）到确切的版本，并补齐了 `go install` 所缺少的管理命令：`list`/`check` 已安装的内容、`remove` 二进制文件、`export`/`import` 整套工具以便在另一台机器上重现它，以及把它 `migrate` 到新的 `$GOBIN`。可在 Windows、macOS 和 Linux 上运行。
...
## 如何安装
除 `go install` 和 Homebrew 外，gup 也已可通过 `winget`、`mise`、`nix` 和 `aqua` 安装。
...
### 使用 aqua
gup 已注册到 [aqua](https://aquaproj.github.io/) 标准注册表。将其添加到您的 `aqua.yaml`：
```shell
...
当某个全局工具必须保持在特定版本时，请使用 `pin`，例如当它需要与 CI 或团队级开发环境保持一致时。
...
被固定的工具会以记录的版本安装（`go install <import_path>@<version>`），绝不会使用 `@latest`。`gup update` 会让它保持在该版本，并在已安装版本与之不同时在该版本重新安装它；其余的工具集仍照常更新。该固定锁定的是模块版本，而不是 Go 构建，因此当 Go 工具链发生变化时，被固定的工具仍会在固定版本上重新构建（与未固定的工具完全一样，可使用 `--ignore-go-update` 抑制该行为）。该固定会以 `channel: "pinned"` 保存在 `gup.json` 中：
...
`gup pin` 也接受 `tool@version` 形式（`gup pin golangci-lint@v1.62.0`）。该工具必须已安装在 `$GOBIN` 下。要再次允许该工具更新：

```shell
$ gup unpin golangci-lint
```

当被固定的工具处于固定版本且以当前 Go 工具链构建时，`gup check` 会将其报告为 `pinned`；当已安装版本不同或有待进行的 Go 工具链重新构建时，则报告为 `pin-mismatch`（并附带 `gup update <name>` 建议）；它绝不会将被固定的工具与 `@latest` 进行比较。
...
每个元素都有以下字段：`name`、`import_path`、`module_path`、`channel`（`latest`/`main`/`master`/`pinned`）、`current_version`、`latest_version`（`list` 时以及被固定的软件包为空）、`pinned_version`（仅在 `channel: "pinned"` 时出现）、`current_go_version`、`installed_go_version`、`status`、`error`（不存在时省略），以及 `hint`（后续步骤建议，仅当某条建议适用于该错误时才出现）。`status` 可以是 `installed`（list）、`up-to-date`、`update-available`（check）、`updated`（update）、`pinned`/`pin-mismatch`（被固定的软件包处于 / 偏离其固定版本）或 `error`。
...
`gup.json` 保存每个工具的 import path、记录的二进制 `version` 和更新 `channel`（`latest` / `main` / `master` / `pinned`）。对于 `channel: "pinned"`，`version` 是该工具被固定到的确切目标版本；对于其他通道，则是导出时记录的版本。`import` 会按文件中记录的确切版本进行安装，且被固定的软件包在导入后仍保持固定。
...
对于没有任何被固定软件包的配置，`schema_version` 为 `1`；一旦有任何软件包被固定，则为 `2`，因此不使用任何固定的环境会继续生成较旧的 gup 版本能够读取的 `1` 格式。gup 同时读取 `1` 和 `2`。`pinned` 通道仅在 `schema_version: 2` 下有效；`schema_version: 1` 下的 `pinned` 条目、没有具体版本的被固定软件包、未知的通道值，或不受支持的 `schema_version` 都会被拒绝。

格式错误或无效的 `gup.json`（无效的 JSON、未知的通道、不受支持的 `schema_version`，或不安全的固定）会被视为错误，而不是静默忽略：`check`、`update` 和 `export` 会立即失败并指出有问题的文件，因此已保存的按包通道绝不会因为配置无法解析而被悄悄降级为 `latest`。未知的通道绝不会被归一化为 `latest`。
...
## gup vs. `go tool`
Go 1.24 内置的 [`go tool`](https://go.dev/doc/modules/managing-dependencies#tools) 管理的是限定于单个项目、并记录在该项目 `go.mod` 中的工具，因此这些工具只存在于该模块内部。gup 管理的是在 `$GOBIN` 下系统级安装的二进制文件，即您可以在任意目录运行、并与 dotfiles 一同保存的命令，并可选择性地将其固定到您所依赖的版本。请将 `go tool` 用于按项目的工具链，将 gup 用于您的全局工具箱。
...
*更新耗时：9 个二进制，每个都有新版本可用；gup 并行，其余顺序执行。AMD Ryzen AI Max+ 395 / go 1.26.4，模块缓存预热后 5 次运行的中位数；耗时取决于构建时间和 CPU。*
