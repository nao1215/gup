...
## gup vs. `go tool`
Встроенный в Go 1.24 [`go tool`](https://go.dev/doc/modules/managing-dependencies#tools) управляет инструментами в рамках одного проекта, записанными в `go.mod` этого проекта, поэтому такие инструменты существуют только внутри этого модуля. gup управляет бинарными файлами, установленными системно в `$GOBIN`, — командами, которые вы запускаете из любого каталога. Используйте `go tool` для инструментов уровня проекта, а gup — для вашего глобального набора инструментов.
