...
gup は `$GOBIN` 配下にインストールされた、グローバルなGoのコマンドラインツールを更新・管理します。`go install` は各プログラムを `$GOBIN`（`$GOPATH/bin`）に配置しますが、その後二度と更新してくれず、何をインストールしたかのマニフェストも残さず、依存するバージョンにツールを固定する手段も提供しません。gup はそのツール一式を管理します。一式をまとめて並列で最新化し、選択したツールを `pin` で正確なバージョンに固定でき、さらに `go install` に欠けている管理用コマンドも提供します。インストール済みのツールを `list`／`check` で一覧・確認し、`remove` でバイナリを削除し、`export`／`import` でツール一式を別マシンに再現し、`migrate` で新しい `$GOBIN` へ移行できます。Windows、macOS、Linuxで動作します。

## gup vs. `go tool`
Go 1.24 に組み込まれた [`go tool`](https://go.dev/doc/modules/managing-dependencies#tools) は、単一のプロジェクトにスコープされ、そのプロジェクトの `go.mod` に記録されたツールを管理するため、それらのツールはそのモジュール内にのみ存在します。一方 gup は、`$GOBIN` 配下にシステム全体でインストールされたバイナリ、つまりどのディレクトリからでも実行でき、dotfiles とともに管理し、必要に応じて依存するバージョンに固定（pin）するコマンドを管理します。プロジェクトごとのツールには `go tool` を、グローバルなツール一式には gup を使い分けてください。
...
| --- | :-: | :-: | :-: |
| 並列更新 | はい | いいえ | 手動 |
| 更新時間（バイナリ9個） | 0.7s | 2.9s | 2.9s |
| パッケージごとの更新チャネル（`latest`/`main`/`master`） | はい | いいえ | 手動 |
| バージョンのピン留め／ロック | はい | いいえ | 手動 |
...
| 失敗時の診断／次の手順のヒント | はい | はい | いいえ |
| `NO_COLOR` 対応 | はい | はい | — |

*更新時間: 新しいバージョンが利用可能な9個のバイナリを更新。gupは並列、他は逐次。AMD Ryzen AI Max+ 395 / go 1.26.4、ウォームキャッシュで5回計測の中央値。時間はビルド時間とCPUに依存します。*
...
```
ソースからのビルドには Go 1.25 以降が必要です。それより古い Go の場合は、代わりにビルド済みのリリースバイナリかパッケージ（後述）をインストールしてください。
...
グローバルなツールを特定のバージョンに留めておく必要がある場合（たとえばCIやチーム全体の開発環境に合わせる必要がある場合）には、`pin` を使用します。
...
固定されたツールは記録されたバージョンでインストールされ（`go install <import_path>@<version>`）、`@latest` でインストールされることはありません。`gup update` はそのバージョンを維持し、インストール済みのバージョンが異なる場合はそのバージョンで再インストールします。それ以外のツール一式は従来どおり更新されます。pin はモジュールのバージョンを固定するもので、Goビルドを固定するわけではないため、Goツールチェインが変わった場合は固定されたツールも固定バージョンのまま再ビルドされます（固定されていないツールと同様に、これを抑制するには `--ignore-go-update` を使用します）。pin は `channel: "pinned"` として `gup.json` に保存されます。
...
`gup pin` は `tool@version` の形式も受け付けます（`gup pin golangci-lint@v1.62.0`）。対象のツールはあらかじめ `$GOBIN` 配下にインストールされている必要があります。ツールを再び更新できるようにするには、次のようにします。

```shell
$ gup unpin golangci-lint
```

`gup check` は、固定されたツールが固定バージョンであり現在のGoツールチェインでビルドされている場合は `pinned` と報告し、インストール済みのバージョンが異なる場合やGoツールチェインの再ビルドが保留中の場合は `pin-mismatch`（`gup update <name>` の提案付き）と報告します。固定されたツールを `@latest` と比較することは決してありません。
...
各要素のフィールドは次のとおりです: `name`、`import_path`、`module_path`、`channel`（`latest`／`main`／`master`／`pinned`）、`current_version`、`latest_version`（`list` および固定されたパッケージでは空）、`pinned_version`（`channel: "pinned"` のときのみ存在）、`current_go_version`、`installed_go_version`、`status`、`error`（無い場合は省略）、`hint`（次の手順の提案。エラーに対して該当するものがある場合のみ存在）。`status` は `installed`（list）、`up-to-date`、`update-available`（check）、`updated`（update）、`pinned`／`pin-mismatch`（固定されたパッケージが固定バージョンである／固定バージョンから外れている）、`error` のいずれかです。
...
`gup.json` は各ツールの import path、記録されたバイナリの `version`、その更新チャネル `channel`（`latest` / `main` / `master` / `pinned`）を保存します。`channel: "pinned"` の場合、`version` はそのツールが固定されている正確なターゲットバージョンです。それ以外のチャネルでは、エクスポート時に記録されたバージョンです。`import` はファイルに書かれた正確なバージョンをインストールし、固定されたパッケージは import 後も固定されたままになります。
...
ユーザーレベルの `gup.json` と `./gup.json` の両方が存在する場合、`import`・`check`・`update` はどちらか一方を暗黙に選ばず、`--file` での明示を促してすぐにエラー終了します。`--file` (`-f`) でパスを上書き指定できます。

`schema_version` は、固定されたパッケージが無い設定では `1`、いずれかのパッケージが固定されると `2` になります。これにより、pin をまったく使わない環境は、古い gup リリースでも読める `1` 形式を生成し続けます。gup は `1` と `2` の両方を読み込めます。`pinned` チャネルは `schema_version: 2` の下でのみ有効です。`schema_version: 1` の下にある `pinned` エントリ、具体的なバージョンを持たない固定パッケージ、未知のチャネル値、サポートされていない `schema_version` は拒否されます。

不正または無効な `gup.json`（不正なJSON、未知のチャネル、サポートされていない `schema_version`、安全でない pin）は、暗黙に無視されるのではなくエラーとして扱われます。`check`・`update`・`export` は即座に失敗し、問題のあるファイル名を示します。これにより、設定をパースできなかったために、保存済みのパッケージごとのチャネルが暗黙に `latest` へ格下げされることはありません。未知のチャネルが `latest` に正規化されることは決してありません。
...
### `gup` が `fatal: not a git repository` で失敗する
おそらく oh-my-zsh を使用しているためです。oh-my-zsh には `git pull --rebase` を実行する `gup` エイリアスが同梱されており、このコマンドを隠してしまいます（[#16](https://github.com/nao1215/gup/issues/16)、[#204](https://github.com/nao1215/gup/issues/204)）。そのエイリアスを削除またはリネームするか、先頭にバックスラッシュを付けて gup を実行して回避してください:
```shell
$ \gup update
```
