...
gup は `$GOBIN` 配下にインストールされた、グローバルなGoのコマンドラインツールを更新・管理します。`go install` は各プログラムを `$GOBIN`（`$GOPATH/bin`）に配置しますが、その後二度と更新してくれません。gup はツール一式をまとめて並列で最新化します。さらに、`go install` に欠けている管理用コマンドも提供します。インストール済みのツールを `list`／`check` で一覧・確認し、`remove` でバイナリを削除し、`export`／`import` でツール一式を別マシンに再現し、`migrate` で新しい `$GOBIN` へ移行できます。Windows、macOS、Linuxで動作します。

## gup vs. `go tool`
Go 1.24 に組み込まれた [`go tool`](https://go.dev/doc/modules/managing-dependencies#tools) は、単一のプロジェクトにスコープされ、そのプロジェクトの `go.mod` に記録されたツールを管理するため、それらのツールはそのモジュール内にのみ存在します。一方 gup は、`$GOBIN` 配下にシステム全体でインストールされたバイナリ、つまりどのディレクトリからでも実行できるコマンドを管理します。プロジェクトごとのツールには `go tool` を、グローバルなツール一式には gup を使い分けてください。
