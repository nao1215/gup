...
## Cómo instalar
gup también está disponible mediante `winget`, `mise`, `nix` y `aqua`, además de `go install` y Homebrew.
...
### Usar aqua
gup está registrado en el registro estándar de [aqua](https://aquaproj.github.io/). Añádelo a tu `aqua.yaml`:
```shell
